...
{
    /*
      一个简单的新浪微博curl模拟登录类. 来源: http://chenall.net/post/sina_curl_login/
      使用方法:
    ...
    */
    ...
    function logout()
    {
      //  request_by_curl("http://weibo.com/logout.php");
      //  unset($this->status);
    }
    ...
    */
    ...
    function login($su,$sp,$flags = 0)
    {
        ...
        $data = request_by_curl("http://login.sina.com.cn/sso/prelogin.php?entry=miniblog&client=ssologin.js&user=".$su);
        if (empty($data))
            return null;
        ...
        $data = request_by_curl("http://login.sina.com.cn/sso/login.php?client=ssologin.js",$data);
        //$this->ResetCookie();
...
$username="wuqifu@pku.edu.cn";
$pwd="wqf363wd";
$sina = new sina($username,$pwd,0);
print_r($sina->status);
/*
if ( $sina->status !=null) {
    echo "login ok\n";
