...
class monitor_main:
    ...
    def __init__(self) :
        ...
        try :
            ...
            self.socket_fd = self.my_socket.connect_server(host, string.atoi(port, 0))
            ...
            if m % 4 == 0 or m == len(dest_str) :
                ...
                for each_value in var_value :
                    ...
                    if each_value[0] == '\033' :
                        ...
                        color_string = '%-19s' % (locale.format('%s', each_value, True))
                        color_string = colored(color_string, 'red', attrs=['bold'])
                        temp_string += color_string
                    else :
                        temp_string += '%-19s' % (locale.format('%s', each_value, True))
...
class cmd_stat(threading.Thread) :
    ...
    def run(self) :
        ...
        while 1 :
            ...
            # it is probably switched off by pressing ENTER when "cmd_stat_switch == 1"
...
def str2ptg(str):
    ...
    grs = [ gr.split(',',1)+[len([ i for i in cnt if i[1]==gr ]),] for gr in grs ]
